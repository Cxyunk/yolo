...
def batched_bincount(index, num_classes, dim=-1):  # 计算index在指定维度上的数量，后续的作用是得到每一个token中心获得的token的数量
    """
...
def center_iter(x, means, buckets=None):
    b, l, d, dtype, num_tokens = *x.shape, x.dtype, means.shape[0]  # 获取元素数量，元素类型，token中心的数量
...
class IASA(nn.Module):
    ...
    def forward(self, normed_x, idx_last, k_global, v_global):
        ...
        paded_q = rearrange(paded_q, "b (ng gs) (h d) -> b ng h gs d", ng=ng,
                            h=self.heads)  # 维度变换从（B,N,C）变换为（B,ng,gs,h,d）
    ...
    def forward(self,x):
        ...
        with torch.no_grad():
             x_scores = torch.einsum('b i c,j c->b i j', 
                                        F.normalize(x, dim=-1), 
                                        F.normalize(x_means, dim=-1))#得到相似度矩阵
...
class LRSA(nn.Module):
    def __init__(self,dim,qk_dim,mlp_dim,heads=1):
        ...
        self.layer = nn.ModuleList([
            PreNorm(dim,CATA_Attention(dim,heads,qk_dim)),
